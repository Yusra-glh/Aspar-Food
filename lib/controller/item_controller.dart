...
class ItemController extends GetxController implements GetxService {
  ...
  List<AddOns> get addOnActiveList => _addOnActiveList;
    ...
    if(cart != null) {
      ...
      item.addOns.forEach((addOn) {
        if(_addOnIdList.contains(addOn.id)) {
             var index =
              _addOnActiveList.indexWhere((element) => element.id == addOn.id);
          if (index == -1) {
            _addOnActiveList.add(addOn);
          } else {
            _addOnActiveList[index] = addOn;
          }
          _addOnQtyList.add(cart.addOnIds[_addOnIdList.indexOf(addOn.id)].quantity);
        }else {
               var index =_addOnActiveList.indexWhere((element) => element.id == addOn.id);
          if (index != -1) {
            _addOnActiveList.removeAt(index);
          }
          _addOnQtyList.add(1);
        }
      });
  ...
  void setAddOnQuantity(bool isIncrement, AddOns addon, List<AddOns> list) {
    var index = list.indexWhere((element) => element.id == addon.id);
    if (isIncrement) {
  ...
  int getQuantity(AddOns addon, List<AddOns> list){
      var index = list.indexWhere((element) => element.id == addon.id);
     return _addOnQtyList[index] ;
  
  }

  void addAddOn(bool isAdd, AddOns addOn) {
    var index =
        _addOnActiveList.indexWhere((element) => element.id == addOn.id);
    if (isAdd) {
      if (index == -1) {
